    ...
    def _generate_crops(self, inputs):
        ...
        for instances, centroid_val, image, fidx, vidx, sz, eff_sc in zip(
            ...
            for instance in valid_instances:
                ...
                image = image if image.dim() == 4 else image.unsqueeze(0)  # Make sure it's (1, C, H, W)
            ...
            for i in range(len(images_to_resize)):
                ...
                eff_scale = torch.tensor(eff_scale, dtype=torch.float32, device=sample_image.device)
...
class FindInstancePeaks(L.LightningModule):
    ...
    def forward(
        ...
        for i in range(peak_points.shape[0]):
            ...
            kpts_homo = torch.cat([kpts, torch.ones((kpts.shape[0], 1), device=kpts.device)], dim=1) 
        ...
        restored_kpts = restored_kpts / (
            inputs["eff_scale"].unsqueeze(dim=1).unsqueeze(dim=2).to(restored_kpts.device)
        )
